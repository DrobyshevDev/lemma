...
Любое заявленное «+3.2%» — это не установленный факт, а утверждение, к которому необходимо задать три вопроса из [модуля 1](01-claim-baseline-noise.md).
...
**С чем сравнили** — база честная или нарочно ослабленная ([модуль 7](07-honest-comparison.md))? **Сколько прогонов** — одно удачное зерно или пять ([модуль 1](01-claim-baseline-noise.md))? **Каков разброс** — доверительные интервалы метода и базы расходятся или перекрываются? Именно здесь чаще всего прячется слабое место: не в самом методе, а в сравнении. Настроенный метод против ненастроенной базы даёт «прирост» из воздуха.
...
**Три вопроса к чужому числу — это [модуль 1](01-claim-baseline-noise.md), повёрнутый наружу.** Там вы задавали их своему результату: с чем сравнил, сколько раз запустил, каков разброс. Здесь те же три вопроса задают чужой статье — и слабое место каждый раз одно и то же, сравнение: настроенный метод против ненастроенной базы ([модуль 7](07-honest-comparison.md)) даёт прирост из воздуха. Читать статью — значит не верить её числу, пока оно не ответит на те же вопросы, что вы задавали своему.
</div>
...
Вы умеете проверить одну статью. В [модуле 25](../programme.md) — как устроена область, из которой эти статьи берутся: arXiv, конференции, рецензирование и бенчмарки, и почему лидерборд — плохой источник истины, которым тем не менее приходится пользоваться. А в [модуле 26](../programme.md) чтение превращается в действие: воспроизведение статьи от заявки до работающего кода.
